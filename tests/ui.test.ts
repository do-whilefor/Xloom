...
describe("TUI command routing", () => {
  ...
  it("routes task/chat resets and settings without sending arguments to an Agent", () => {
    ...
    for (const command of ["/run https://localhost 对比账户", "/new", "/model", "/model decide", "/login", "/login OpenAI Codex", "/logout"]) dispatchCommand(command, app, actions);
    expect(actions.run).toHaveBeenCalledWith("https://localhost 对比账户");
    expect(resetChat).toHaveBeenCalledOnce();
    expect(actions.settings.mock.calls).toEqual([["model", ""], ["model", "decide"], ["login", ""], ["login", "OpenAI Codex"], ["logout", ""]]);
    expect(controller.hint).not.toHaveBeenCalled();
  });
  ...
  it("rejects inline secrets and invalid model roles without echoing or recording secrets", () => {
    ...
    for (const command of ["/apikey anthropic PRIVATE_KEY", "/logout anthropic PRIVATE_KEY", "/model invalid", "/run"]) dispatchCommand(command, controller, actions);
  ...
  it.each(["/apikey", "/details", "/quit"])("rejects removed command %s without side effects", command => {
    const { controller } = fakeController();
